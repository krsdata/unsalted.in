...
class ApiController extends BaseController
{
    ...
    public function getMyTeam(Request $request){
        ...
        foreach ($myTeam as $key => $result) {
           ...
           $t_a = TeamA::WhereIn('team_id',$team_id) 
            ...
            $k['t_img']         = "";
            // username
            $k['team_name'] =  $user_name->first_name. '('.$result->team_count.')';
    ...
    */
    ...
    public function createContest($match_id=null){
        ...
        $c[] = [
                'contest_type' => 1,
                'total_winning_prize'=>30000,
                'entry_fees' => 35,
                'total_spots'   => 5,
                'filled_spot' => 0,
                'first_prize' => 5000,
                'winner_percentage' => 45,
                'cancellation' => 'no'

            ];
        $c[] = [
                'contest_type' => 2,
                'total_winning_prize'=>1700,
                'entry_fees' => 25,
                'total_spots'   => 3,
                'filled_spot' => 0,
                'first_prize' => 500,
        ...
        foreach ($c as $key => $result) {
            ...
            $createContest->filled_spot         =   $result['filled_spot'];
    ...
    public function updateMatchDataByStatus($status=1)
    {
        ...
        $token = "8740931958a5c24fed8b66c7609c1c49";
        //upcoming
        $data =    file_get_contents('https://rest.entitysport.com/v2/matches/?status='.$status.'&token='.$token.'&per_page=10');
          
        \File::put(public_path('/upload/json/'.$fileName.'.txt'),$data);
        
        $this->storeMatchInfo($fileName);
    ...
    public function getMatch(Request $request){
        ...
        if($join_contests){  
            ...
            foreach ($join_contests as $key => $join_contest) {
                # code...
               $joinedmatches = Matches::with('teama','teamb')->where('match_id',$join_contest->match_id)->select('match_id','title','short_title','status','status_str','timestamp_start','timestamp_end')->first();
               
                $jm[] = $joinedmatches;
            }
            $data['matchdata'][] = ['viewType'=>1,'joinedmatches'=>$jm];
            
        }                                          
        $match = Matches::with('teama','teamb')->where('status',1)->select('match_id','title','short_title','status','status_str','timestamp_start','timestamp_end','date_start','date_end')->get();


        $d = \Carbon\Carbon::createFromTime(12, 0, 0, 'Europe/London');
    ...
    //https://rest.entitysport.com/v2/competitions/113466/squads/41958?token=8740931958a5c24fed8b66c7609c1c49
    ...
    public function getWallet(Request $request){
      $myArr = array();
      $myArr['wallet_amount'] = "280.00"; 
      $myArr['bonus_amount'] = "100.00"; 
